    ...
    try:
        ...
        for param in redirect_params[:3]:
            if len(findings) >= 1:
                break
            test_url = (
                url + ("&" if "?" in url else "?") + f"{param}={quote(test_payload)}"
            )
            try:
                r = requests.get(
                    test_url, timeout=5, verify=True, allow_redirects=False
                )
                loc = r.headers.get("Location", "")
                if "evil.com" in loc:
                    findings.append(
                        {
                            "type": f"Open Redirect in param '{param}'",
                            "severity": "MEDIUM",
                            "score": 15,
                            "description": f"Параметр {param} дозволяє редирект на зовнішній домен — фішинг ризик",
                            "fix": "Валідувати redirect URL на whitelist, використовувати відносні шляхи",
