...
def finding_score(finding: Finding) -> int:
    base = finding.score if finding.score and finding.score > 0 else SEVERITY_WEIGHTS.get(finding.severity, 5)
    # OWASP бонус
...
def aggregate_findings(scanner_results: list) -> List[Finding]:
    ...
    for f in all_findings:
        key = (f.type, f.severity.value if hasattr(f.severity, "value") else str(f.severity))
        if key not in seen:
            seen.add(key)
            deduped.append(f)
    return deduped
...
def get_summary(findings: List[Finding]) -> dict:
    ...
    return {"risk_score": score, "level": level.value, "total_findings": len(findings), "by_severity": counts}
